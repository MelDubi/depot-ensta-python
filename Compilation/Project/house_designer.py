...
class HouseDesigner:
    ...
    def visit_kitchen(self, kitchen):
        ...
        DesignerTools.draw_rectangle(self.ax, int(kitchen.lenght.value), int(kitchen.width.value),
                                     label=kitchen.name.value)

    def visit_lounge(self, lounge):
        self.room.append(lounge)
        DesignerTools.draw_rectangle(self.ax, int(lounge.lenght.value), int(lounge.width.value),
                                     label=lounge.name.value)

    def visit_bedroom(self, bedroom):
        self.room.append(bedroom)
        DesignerTools.draw_rectangle(self.ax, int(bedroom.lenght.value), int(bedroom.width.value),
                                     label=bedroom.name.value)

    def visit_bathroom(self, bathroom):
        self.room.append(bathroom)
        DesignerTools.draw_rectangle(self.ax, int(bathroom.lenght.value), int(bathroom.width.value),
                                     label=bathroom.name.value)