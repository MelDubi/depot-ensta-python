...
class Parser:
    ...
    def parse_program(self):
        ...
        self.expect("KW_MAIN")
        self.expect("OPEN_PARENTHESE")
        self.expect("CLOSE_PARENTHESE")
        self.expect("OPEN_CURL_BRACKET")
        program = ast.Program()

        while self.show_next().tag != "CLOSE_CURL_BRACKET":
            if self.show_next().tag in ["TYPE_INT", "TYPE_BOOL", "TYPE_FLOAT", "TYPE_CHAR"]:
    ...
    def parse_declaration(self):
        ...
        if self.show_next().tag == "OPEN HOOK":
            self.accept()
            integer = self.expect("INTEGER")
            self.expect("CLOSE HOOK")
        self.expect("SEMICOLON")
        return ast.Declaration(type, identifier, integer)
